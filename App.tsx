...
import { Recipe, ProcessingState, PlannerData } from './types';
...
const App: React.FC = () => {
  ...
  const [recipes, setRecipes] = useState<Recipe[]>(() => {
    const saved = localStorage.getItem('tokchef_recipes');
    let initialRecipes = saved ? JSON.parse(saved) : MOCK_RECIPES;
  ...
  const [planner, setPlanner] = useState<PlannerData>(() => {
    const saved = localStorage.getItem('tokchef_planner');
  ...
  useEffect(() => {
    localStorage.setItem('tokchef_recipes', JSON.stringify(recipes));
  }, [recipes]);

  useEffect(() => {
    localStorage.setItem('tokchef_planner', JSON.stringify(planner));
  }, [planner]);
