...
// Netlify Function wrapper of the existing api/extract.cjs logic (Gemini + fallback)
...
exports.handler = async (event) => {
  ...
  // Same behavior as your local handler: no key => mock :contentReference[oaicite:1]{index=1}
  if (!apiKey) {
    return json(200, { ok: true, source: "mock", recipe: MOCK_RECIPE });
  }
  ...
  // --- Build best possible input context (same flow) :contentReference[oaicite:2]{index=2}
  let creator = null;
  let thumbnail_url = null;

  let inputText = text || caption || url || "Extract a cooking recipe.";

  // If caller only sends URL, try TikTok oEmbed for context :contentReference[oaicite:3]{index=3}
  ...
  if (!lm.ok || lm.models.length === 0) {
    return json(200, { ok: true, source: "mock-fallback", recipe: MOCK_RECIPE });
  }
  ...
  for (const model of models) {
    ...
    if (textOut) {
      // Defensive: strip possible ```json fences :contentReference[oaicite:4]{index=4}
